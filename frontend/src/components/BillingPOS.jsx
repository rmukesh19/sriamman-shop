            ...
            <table className="w-full text-left text-xs font-semibold">
              <thead className="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider sticky top-0 border-b border-slate-100 z-10">
                <tr>
                  ...
                  <th className="p-2 text-center w-24">Pack / Method</th>
                  <th className="p-2 text-center w-16">Rate</th>
                  <th className="p-2 text-center w-14">Bags/Qty</th>
                  ...
                  <tr key={idx} className="hover:bg-slate-50/50">
                    <td className="p-2">
                      <p className="font-bold text-slate-800 text-[11px] truncate max-w-[120px]">
                        {language === "English" ? item.englishName : item.tamilName}
                      </p>
        ...
        <div id="quick-customer-modal" className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex justify-center items-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl relative border border-slate-100">
            ...
            <form onSubmit={handleQuickCustomerSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-[9px] font-black text-slate-500 uppercase tracking-wider mb-1">பெயர் (தமிழ்) - Optional</label>
                <input
                  type="text"
                  value={newCustTamil}
              ...
              <div>
                <label className="block text-[9px] font-black text-slate-500 uppercase tracking-wider mb-1">Delivery Address - Optional</label>
                <textarea
                  value={newCustAddress}
                  onChange={(e) => setNewCustAddress(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-xs font-bold focus:outline-none focus:bg-white h-16 uppercase"
                />
              </div>
