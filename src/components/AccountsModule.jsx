...
const AccountsModule = ({
  ...
  // Calculated Datasets
  const getCustomerOutstanding = useCallback((cust) => {
    if (!cust) return 0;
    const custBills = bills.filter(
      (b) => (b.customerId === cust.id || (cust.name && b.customerName && b.customerName.toLowerCase() === cust.name.toLowerCase())) && b.status !== "Cancelled"
    );
  ...
  const getSupplierOutstanding = useCallback((supp) => {
    if (!supp) return 0;
    const suppPurchases = purchases.filter(
      (p) => p.supplierId === supp.id || (supp.name && p.supplierName && p.supplierName.toLowerCase() === supp.name.toLowerCase())
    );
    const purchaseOutstanding = suppPurchases.reduce((sum, p) => {
      const tot = p.grandTotal !== undefined ? p.grandTotal : p.total || 0;
      const paid = p.paidAmount !== undefined ? p.paidAmount : 0;
      return sum + Math.max(0, tot - paid);
    }, 0);
    return Math.max(Number(supp.outstanding) || 0, purchaseOutstanding);
  }, [purchases]);
  ...
  const customerBills = useMemo(() => {
    ...
    return bills.filter(
      (b) => (b.customerId === selectedCustomer.id || (selectedCustomer.name && b.customerName && b.customerName.toLowerCase() === selectedCustomer.name.toLowerCase())) && b.status !== "Cancelled"
    );
  ...
  const supplierPurchases = useMemo(() => {
    ...
    return purchases.filter(
      (p) => p.supplierId === selectedSupplier.id || (selectedSupplier.name && p.supplierName && p.supplierName.toLowerCase() === selectedSupplier.name.toLowerCase())
    );
  ...
  const totalCustomerOutstanding = useMemo(() => customers.reduce((s, c) => s + getCustomerOutstanding(c), 0), [customers, getCustomerOutstanding]);
  const totalSupplierOutstanding = useMemo(() => suppliers.reduce((s, sObj) => s + getSupplierOutstanding(sObj), 0), [suppliers, getSupplierOutstanding]);
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                >
                  {customers.map((c) => {
                    const os = getCustomerOutstanding(c);
                    return (
                      <option key={c.id} value={c.id}>
                        {c.name} {os > 0 ? `(O/S: ₹${os.toLocaleString("en-IN")})` : ""}
                      </option>
                    );
                  })}
                </select>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    ...
                    <span className="text-lg font-black text-slate-800 block mt-1">
                      {fmt(customerBills.reduce((s, b) => s + (b.grandTotal !== undefined ? b.grandTotal : b.total || 0), 0))}
                    </span>
                  </div>
                  <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-200">
                    ...
                    <span className="text-lg font-black text-emerald-700 block mt-1">
                      {fmt(customerBills.reduce((s, b) => s + (b.paidAmount !== undefined ? b.paidAmount : (b.paymentType === "Credit" ? 0 : (b.grandTotal || b.total || 0))), 0))}
                    </span>
                  </div>
                  <div className="bg-rose-50/50 p-4 rounded-xl border border-rose-200 flex justify-between items-center">
                    <div>
                      ...
                      <span className="text-xl font-black text-rose-700 block mt-1">
                        {fmt(getCustomerOutstanding(selectedCustomer))}
                      </span>
                    </div>
                    {getCustomerOutstanding(selectedCustomer) > 0 && (
                      <button
                        onClick={() => {
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {customerBills.length === 0 ? (
                        ...
                        customerBills.map((b, idx) => {
                          const tot = b.grandTotal !== undefined ? b.grandTotal : b.total || 0;
                          const paid = b.paidAmount !== undefined ? b.paidAmount : (b.paymentType === "Credit" ? 0 : tot);
                          const bal = b.balance !== undefined ? b.balance : Math.max(0, tot - paid);
                          return (
                            <tr key={b.id || idx} className="hover:bg-slate-50/50">
                              ...
                              <td className="p-3 text-right font-black text-slate-800">{fmt(tot)}</td>
                              <td className="p-3 text-right text-emerald-600 font-bold">{fmt(paid)}</td>
                              <td className="p-3 text-right text-rose-600 font-black">{fmt(bal)}</td>
                            </tr>
                          );
                        })
                      )}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                >
                  {suppliers.map((s) => {
                    const os = getSupplierOutstanding(s);
                    return (
                      <option key={s.id} value={s.id}>
                        {s.name} {os > 0 ? `(Payable: ₹${os.toLocaleString("en-IN")})` : ""}
                      </option>
                    );
                  })}
                </select>
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    ...
                    <span className="text-lg font-black text-slate-800 block mt-1">
                      {fmt(supplierPurchases.reduce((s, p) => s + (p.grandTotal !== undefined ? p.grandTotal : p.total || 0), 0))}
                    </span>
                  </div>
                  <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-200">
                    <span className="text-[10px] font-bold text-emerald-600 uppercase">Total Payments Made</span>
                    <span className="text-lg font-black text-emerald-700 block mt-1">
                      {fmt(supplierPurchases.reduce((s, p) => s + (p.paidAmount || 0), 0))}
                    </span>
                  </div>
                  <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-200 flex justify-between items-center">
                    <div>
                      ...
                      <span className="text-xl font-black text-amber-700 block mt-1">
                        {fmt(getSupplierOutstanding(selectedSupplier))}
                      </span>
                    </div>
                    {getSupplierOutstanding(selectedSupplier) > 0 && (
                      <button
                        onClick={() => {
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4">
            ...
            <div className="overflow-x-auto border border-slate-200 rounded-xl">
              <table className="w-full text-left text-xs font-semibold">
                ...
                <tbody className="divide-y divide-slate-100">
                  {customers.filter((c) => getCustomerOutstanding(c) > 0).length === 0 ? (
                    <tr>
                      <td colSpan={4} className="p-4 text-center text-slate-400 italic">No customer outstanding balance! 🎉</td>
                    </tr>
                  ) : (
                    customers
                      .filter((c) => getCustomerOutstanding(c) > 0)
                      .map((c, idx) => {
                        const due = getCustomerOutstanding(c);
                        return (
                          <tr key={c.id || idx} className="hover:bg-slate-50/50">
                            <td className="p-2.5 font-bold text-slate-800">{c.name}</td>
                            <td className="p-2.5 text-slate-500">{c.phone || "N/A"}</td>
                            <td className="p-2.5 text-right font-black text-rose-600">{fmt(due)}</td>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4">
            ...
            <div className="overflow-x-auto border border-slate-200 rounded-xl">
              <table className="w-full text-left text-xs font-semibold">
                ...
                <tbody className="divide-y divide-slate-100">
                  {suppliers.filter((s) => getSupplierOutstanding(s) > 0).length === 0 ? (
                    <tr>
                      <td colSpan={4} className="p-4 text-center text-slate-400 italic">No supplier payable balance! 🎉</td>
                    </tr>
                  ) : (
                    suppliers
                      .filter((s) => getSupplierOutstanding(s) > 0)
                      .map((s, idx) => {
                        const due = getSupplierOutstanding(s);
                        return (
                          <tr key={s.id || idx} className="hover:bg-slate-50/50">
                            <td className="p-2.5 font-bold text-slate-800">{s.name}</td>
                            <td className="p-2.5 text-slate-500">{s.phone || "N/A"}</td>
                            <td className="p-2.5 text-right font-black text-amber-600">{fmt(due)}</td>
                            <td className="p-2.5 text-center">
                              <button
                                onClick={() => {
                                  setSuppPayId(s.id);
            ...
            <form onSubmit={handleSaveIncome} className="space-y-3 text-xs font-semibold">
              ...
              <div>
                ...
                >
                  ...
                  {accountsLedgers
                    .filter((l) => (l.type || l.groupType) === "Income" && !["General Income", "Commission Received", "Rent Received", "Scrap Sales", "Interest Received"].includes(l.name))
                    .map((l) => (
            ...
            <form onSubmit={handleSaveExpense} className="space-y-3 text-xs font-semibold">
              ...
              <div>
                ...
                >
                  ...
                  {accountsLedgers
                    .filter((l) => (l.type || l.groupType) === "Expense" && !["Shop Rent", "Electricity Bill", "Labor & Loading Charges", "Vehicle Transport", "Tea & Refreshments", "Maintenance & Repair", "General Expenses"].includes(l.name))
                    .map((l) => (
                      <option key={l.id} value={l.name}>{l.name}</option>
                    ))}
                </select>
